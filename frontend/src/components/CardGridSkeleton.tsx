  ...
  return (
    <div className="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-3 sm:grid-cols-[repeat(auto-fill,minmax(170px,1fr))] sm:gap-4">
      {Array.from({ length: count }).map((_, index) => (
        <div
          key={index}
          className="flex flex-col overflow-hidden rounded-lg border border-pokedex-border bg-pokedex-surface-2"
        >
...
export function CardCarouselSkeleton() {
  return (
    <div className="flex flex-col gap-3">
      <div className="mx-auto flex w-[86%] flex-col gap-3 rounded-lg border border-pokedex-border bg-pokedex-surface-2 p-3">
        <div className="aspect-[3/4] w-full animate-pulse rounded-md bg-pokedex-border/60" />
