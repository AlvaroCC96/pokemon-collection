...
import { ModeTag, PokedexHeader, PokedexScreen, PokedexShell, ScanlineTopBar, SectionLabel } from '../components/Pokedex'
import { LANGUAGE_OPTIONS } from '../utils/languages'

const inputClass =
  'w-full rounded-xl border border-pokedex-border bg-pokedex-surface px-3 py-2 text-sm text-pokedex-text placeholder:text-pokedex-muted focus:border-pokedex-red/50 focus:outline-none focus:ring-2 focus:ring-pokedex-red/15'
  ...
  return (
    <PokedexShell>
      ...
      <PokedexScreen>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,380px)_1fr] lg:items-stretch">
          <form
          ...
          >
            ...
            <div className="flex flex-col gap-4 p-5 sm:p-6">
              ...
              <button
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-pokedex-red px-5 py-2.5 text-sm font-semibold uppercase tracking-wide text-white transition hover:bg-pokedex-red/85 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {searching && (
        ...
        {result && result.status === 'multiple_matches' && (
          <section className="flex flex-col gap-4 rounded-2xl border border-pokedex-border bg-pokedex-surface-2/60 p-5">
            <div className="flex items-center justify-between">
              <SectionLabel color="gold">Matches</SectionLabel>
              <span className="rounded-full bg-pokedex-bg px-2.5 py-0.5 text-xs font-medium text-pokedex-text/80">
            ...
            {result.has_more && (
              <button
                ...
                className="inline-flex w-fit items-center gap-2 self-center rounded-full border border-pokedex-border px-4 py-1.5 text-sm text-pokedex-text/80 transition hover:bg-pokedex-surface disabled:cursor-not-allowed disabled:opacity-60"
              >
                {loadingMore && (
